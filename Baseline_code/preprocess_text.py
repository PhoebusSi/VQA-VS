...
if __name__ == '__main__':
    ...
    train_answer_file =  './VQA-VS/Training/Training-Ans.json'
    train_answers = json.load(open(train_answer_file))

    val_answer_file =  './VQA-VS/Val/Val-Ans.json'
    val_answers = json.load(open(val_answer_file))

    # test_answer_file =  './VQA-VS/IID-Test/IID-Test-Ans.json'
    # test_answers = json.load(open(test_answer_file))

    train_question_file =  './VQA-VS/Training/Training-Ques.json'
    train_questions = json.load(open(train_question_file))

    val_question_file = './VQA-VS/Val/Val-Ques.json'
    val_questions = json.load(open(val_question_file))

    test_question_file = './VQA-VS/IID-Test/IID-Test-Ques.json'
    test_questions = json.load(open(test_question_file))

    answers = train_answers
    occurence = filter_answers(answers, 9)

    cache_path = './cache/train_val_ans2label.pkl'
    if os.path.isfile(cache_path):
        print('found %s' % cache_path)
        ans2label = cPickle.load(open(cache_path, 'rb'))
    else:
        ans2label = create_ans2label(occurence, 'train_val', './cache')

    target_root='./cache'
    compute_target(train_answers, ans2label, 'train', target_root)
    compute_target(val_answers, ans2label, 'val', target_root)
    # compute_target(test_answers, ans2label, 'test', target_root)
    # compute_target_for_testing(test_answers, ans2label, 'test', target_root)
